...
CHR_CURRENT_ENUM g_temp_standar_charger_value = CHARGE_CURRENT_550_00_MA;
...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) {
		...
		if ((g_temp_status == TEMP_ABOVE_POS_60)
			...
			battery_log(BAT_LOG_CRTI,
				    "[BATTERY] 111 Battery Temperature between %d and %d !!\n\r",
	   ...
	   if (BMT_status.charger_type == STANDARD_HOST){
		  g_temp_CC_value = USB_CHARGER_CURRENT_CONFIGURED;
		   ...
		   //yixuhong 20151116 add for test
				...
				{
					g_temp_CC_value = CHARGE_CURRENT_700_00_MA;//g_temp_standar_charger_value;
				}
