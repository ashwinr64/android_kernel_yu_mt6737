...
//caizhifu modified  start,mark mtk battery data and add ours battery data, 2016-10-27
//#include <mach/mt_battery_meter.h>
...
#endif
//caizhifu modified end ,mark mtk battery data and add ours battery data,  2016-10-27
...
//system a long time sleep, the soc cannot update 
...
//bq24296 charger ic: not use rsense, directly use Vbat to calculation init soc
#ifdef GET_OCV_FROM_PL
	int ibat = 0;
	int vbat = 0;
	int rbat = 0;	//battery internal resistor
	int real_current = 0;
...
// Added by liaoye on Nov. 22, 2016 for: compensation soc, get ocv from preloader
#ifdef GET_OCV_FROM_PL
	pr_debug("get_ocv_from_pl: ibat = %d, vbat = %d, %d, %d, %d, %d\n", 
...
// according the temperature to select the bat capacity, start
	gFG_BATT_CAPACITY_aging =
		fgauge_get_Q_max(battery_meter_get_battery_temperature());
#ifdef CAPACITY_TUNE_RATIO 
	gFG_BATT_CAPACITY_aging = gFG_BATT_CAPACITY_aging * CAPACITY_TUNE_RATIO / 100;
#endif
// TINNO END
...
// TINNO END
	...
	bm_print(BM_LOG_FULL, "[oam_run] %d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n",
		 oam_i_1, oam_i_2, oam_car_1, oam_car_2, oam_d_1, oam_d_2, oam_v_ocv_1, oam_d_3,
		 oam_r_1, oam_v_ocv_2, oam_r_2, vol_bat, g_vol_bat_hw_ocv, g_d_hw_ocv, gFG_BATT_CAPACITY_aging);
