...
static kal_bool mt_battery_100Percent_tracking_check(void)
{
	...
	if (BMT_status.bat_full == KAL_TRUE) {	/* charging full first, UI tracking to 100% */
		...
		battery_log(BAT_LOG_CRTI, "[100percent], UI_SOC(%d), reset(%d) bat_full(%d) ever100(%d) SOC:%d\n",
			    BMT_status.UI_SOC, resetBatteryMeter, BMT_status.bat_full, is_uisoc_ever_100,BMT_status.SOC);
		...
		if (BMT_status.UI_SOC >= 99) {
			...
			battery_log(BAT_LOG_CRTI, "[100percent],UI_SOC = %d,SOC = %d\n", BMT_status.UI_SOC, BMT_status.SOC);
		}
...
#if defined(CONFIG_TINNO_PG_PROTECT)
#define MAX_CHARGING_PG 1200000	//LINE <GGABM-400> <optimze the cc  current charge period>  <20160606> panzaoyan  
...
// TINNO BEGIN
// Added by liaoye on Nov. 14, 2016 for bugid:DHACN-857
...
// TINNO BEGIN
// Added by liaoye on Nov. 14, 2016 for: update the UI_SOC immediately after system resume.
#ifdef FEATURE_SLEEP_SYNC_SOC
...
// TINNO END
	...
	battery_xlog_printk(BAT_LOG_CRTI, "geroge maxPgFlag start =%d, g_temp_standar_charger_value=%d v=%d ,ICharging = %d \n", maxPgFlag,g_temp_standar_charger_value,v,ICharging);
	if((ICharging>0)&(v>0)&(pg>1000))
	{
	    batteryAveragePg=mt_battery_average_method(3,&batteryPgBuffer[0], pg, &pg_sum, batteryPgIndex);
	    batteryPgIndex++;
        if (batteryPgIndex >= BATTERY_AVERAGE_SIZE)
            batteryPgIndex = 0;
	    battery_xlog_printk(BAT_LOG_CRTI, "geroge maxPgFlag=%d, batteryAveragePg=%d, pg=%d, \n",maxPgFlag, batteryAveragePg,pg);
	}
...
battery_xlog_printk(BAT_LOG_CRTI, "geroge maxPgFlag start =%d \n", maxPgFlag);
#endif
...
// TINNO BEGIN
// Added by liaoye on Nov. 14, 2016 for bugid:DHACN-857
// inhibit the soc drop during charging
...
// TINNO END
	...
	battery_log(BAT_LOG_CRTI, "v=%d,i=%d,t=%d,soc=%d,bcct:%d:%d I:%d Ibat:%d\n",
		    bat_vol, ICharging, temperature, BMT_status.SOC, g_bcct_flag,
