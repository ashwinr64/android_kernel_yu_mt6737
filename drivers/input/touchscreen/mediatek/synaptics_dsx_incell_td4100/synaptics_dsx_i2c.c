...
#define DRIVICE_NAME "TD4100"

#ifdef KERNEL_ABOVE_2_6_38
#define TYPE_B_PROTOCOL
#endif

#define UBL_I2C_ADDR 0x72
...
exit:
	...
	kfree(ctrl_8);
	kfree(ctrl_23);
	printk("synaptics end 12:fhandler->num_of_data_sources =%d, fhandler->fn_number=%d\n",fhandler->num_of_data_sources,fhandler->fn_number);
	return retval;
}
...
//add touch info 
static ssize_t synaptics_rmi4_show_tpfwversion( char * fw_vendor_numb )
{
        int fw_version = 0;
        fw_version=synaptics_get_fw_version();

        return snprintf(fw_vendor_numb, PAGE_SIZE, "%d\n",fw_version);
}

static ssize_t synaptics_rmi4_show_tpvendor(char * fw_vendor_name)
{	
	char *product_info = synaptics_get_vendor_info();
	if(product_info)
	{
		if( 0 == memcmp(product_info, "TD4100test", 10))
		{
			sprintf(fw_vendor_name, "%s\n", "BOE");
		}
		else
		{
			sprintf(fw_vendor_name, "%s\n", product_info);
...
static void synaptics_ftm_force_update(char * ftm_update)
{
	printk("synaptics synaptics_ftm_force_update\n");
	ftm_update_FW();
}
...
static struct i2c_driver tpd_i2c_driver = {
	.driver = {
        	 .name = DRIVICE_NAME,//"synaptics_dsx",
...
static struct tpd_driver_t synaptics_rmi4_driver = {
	.tpd_device_name = DRIVICE_NAME,//"synaptics_dsx",
	.tpd_local_init = tpd_local_init,
	.suspend = synaptics_rmi4_suspend,
	.resume = synaptics_rmi4_resume,
	.tpd_get_fw_version = synaptics_rmi4_show_tpfwversion,
    	.tpd_get_fw_vendor_name = synaptics_rmi4_show_tpvendor,
