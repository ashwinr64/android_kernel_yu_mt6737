...
static int gconfig_id = 0;//fyf add
static bool ftm_force_update_fw = false;
extern int synaptics_read_fw_version(void);
...
static enum flash_area fwu_go_nogo(void)
{
	...
	if (ftm_force_update_fw)
	{
		printk("synaptics ftm force update fw\n");
		ftm_force_update_fw = false;
...
void ftm_update_FW(void)
{	
	ftm_force_update_fw = true;
 	synaptics_fw_updater(FirmwareImage);
	printk("synaptics ftm_update_FW \n");
	return;
}
EXPORT_SYMBOL(ftm_update_FW);
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	{
		dev_err(&fwu->rmi4_data->i2c_client->dev,
					"%s: Is not normal boot,no update fw\n",
					__func__);
		return;
	}
...
//add for query tp information
int synaptics_read_fw_version(void)
{
	int ret;
