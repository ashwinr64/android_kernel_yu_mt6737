...
#ifdef TINNO_TCL_HSD_CTP
	else if ( 0 == memcmp(tpd_desc, "HSD", 3))
	{
		ft6x06_tp_upgrade(ftbin_hsd, sizeof(ftbin_hsd));
	}
#endif /*TINNO_TCL_HSD_CTP*/
}else{
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (get_incell_lcm_connect()){
		CTP_DBG("oncell touch not connect\n");
    ...
    */
    ...
    CTP_DBG("HJDDbgTP, Touch Panel Device(%s), panel_vendor=0x%x, Probe PASS\n", fts_get_vendor_name(panel_vendor), panel_vendor);
...
#if defined (BACKBUTTON_FUNCTION)
    const char dataEnDoubleclick = 0x30;
#else
