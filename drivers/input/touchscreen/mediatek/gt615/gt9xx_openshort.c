...
extern u8 cfg_len;
extern int tpd_halt;
static bool gtp_test_sysfs = false;
...
extern u16 aver_standard[MAX_DRIVER_NUM];
...
#if 0                  
#define SET_INFO_LINE_INFO(fmt, args...)      printk("<<-GTP-INFO->> "fmt"\n",##args) //do{} while(0)
                                                   
#define SET_INFO_LINE_ERR(fmt, args...)        printk("<<-GTP-INFO->> "fmt"\n",##args) //do {} while(0)
#else
...
static void gtp_raw_min_test(u16 *raw_buf)
{
    ...
    for (i = gt9xx_sen_num; i < (gt9xx_sc_pxl_cnt - gt9xx_sen_num); i++)
    {
    	driver = (i/gt9xx_sen_num);
      if ((raw_buf[i] > ( aver_standard[driver]* (10-RAW_P1)/10))&&(raw_buf[i] < ( aver_standard[driver]* (10+RAW_P1)/10)))
	{
            sum_all_driv+= raw_buf[i];
	     	real_node_num++;
        }
    }
	
    IC_Aver = sum_all_driv/real_node_num;
   	sum_all_driv = 0;
	real_node_num = 0;
	for (i = 1; i < (gt9xx_drv_num -1); i++)
	{
		sum_all_driv+= aver_standard[i];
	 	real_node_num++;
	}
	set_driv_aver = sum_all_driv/real_node_num;
	RAW_P3 =set_driv_aver*1000/IC_Aver;
	...
	for (i = 0; i < gt9xx_sc_pxl_cnt; i++)
	{
		...
		if ((raw_buf[i] * RAW_P3/1000) < ( aver_standard[driver] * (10-RAW_P2)/10 ))			
		{
...
{   
	...
	GTP_DEBUG("check_types:%d,MAX:%d,MAX:%d",check_types,(check_types & _MAX_TEST),(check_types & _MIN_TEST));
...
static s32 gtp_get_test_result(void)
{
	...
	SET_INFO_LINE_INFO("beyond_uniformity:%d,_MIN_ERROR_NUM:%d",beyond_uniformity,_MIN_ERROR_NUM);
...
static ssize_t gtp_sysfs_opentest_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	if (ret == SUCCESS)
	{
		len = sprintf(buf, "GOODIX PASS \n");
	}
	else
	{
		len = sprintf(buf, "GOODIX FAIL \n");
	}
