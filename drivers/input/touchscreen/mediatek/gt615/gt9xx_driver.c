...
#if defined(CONFIG_TP_AUTO_TEST)
...
static void update_tp_button_dim(void)
{
...
static s32 gtp_init_panel(struct i2c_client *client)
{
...
#endif
	{
		...
		if (ret == SUCCESS) {
	        ...
	        if (flash_cfg_version < 0x90 && flash_cfg_version > drv_cfg_version) {
...
static int touch_event_handler(void *unused)
{
	...
	do {
...
#if defined(CONFIG_GTP_SLIDE_WAKEUP)
        if(tpd_halt == 1)// (DOZE_ENABLED == doze_status)
        {
            ret = gtp_i2c_read(i2c_client_point, doze_buf, 3);
	#if defined (BACKBUTTON_FUNCTION)		
	    ret = gtp_i2c_read(i2c_client_point, data_buf, 3);
	    GTP_DEBUG("0x8155 = 0x%02X",data_buf[2]);
	#endif
	...
	#if defined (BACKBUTTON_FUNCTION)
		else if ((0xCC == doze_buf[2])&&(data_buf[2] == 0x01)){
			GTP_INFO("Backbutton to light up the screen!");
			   doze_status = DOZE_WAKEUP;
		...
		else if ((0xCC == doze_buf[2])&&(data_buf[2] == 0x0f)) // double click wakeup
                {
		...
		GTP_DEBUG("use_tpd_button =%d",tpd_dts_data.use_tpd_button);
		if (tpd_dts_data.use_tpd_button) {
...
#if defined(CONFIG_GTP_SLIDE_WAKEUP)
GTP_DEBUG("rbEnTGesture:%d ",bEnTGesture);
...
#endif
        GTP_DEBUG("bEnTGesture:%d ",bEnTGesture);
#if defined(CONFIG_GTP_SLIDE_WAKEUP)
	if (bEnTGesture == 1)
	{
