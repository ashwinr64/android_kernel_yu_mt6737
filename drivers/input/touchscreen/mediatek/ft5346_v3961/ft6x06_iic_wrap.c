...
int fts_i2c_global_read(char *writebuf,int writelen, char *readbuf, int readlen)
{
	int ret,i;
	...
	if(writelen!=0)
	{
		...
		if((ret=i2c_master_send(g_pts->client, (unsigned char *)tpDMABuf_pa, writelen))!=writelen)
...

	{
		...
		ret = i2c_master_recv(g_pts->client, (unsigned char *)tpDMABuf_pa, readlen);
