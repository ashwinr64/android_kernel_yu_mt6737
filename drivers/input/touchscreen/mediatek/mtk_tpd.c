...
struct pinctrl_state *eint_as_int, *eint_output0, *eint_output1, *rst_output0, *rst_output1, *power_outputl, *power_outputh;
...
void tpd_gpio_output(int pin, int level)
{
    ...
    {
    	if (get_incell_lcm_connect())
    	{
    		printk ("is incell touch\n");
    	}else{
...
void tpd_power_ldo(int onoff)
{
#if  (defined(CONFIG_PROJECT_V3991) || defined(CONFIG_PROJECT_V3995))
if (get_incell_lcm_connect()==false){
#endif
    TPD_DEBUG("[tpd]tpd_power_ldo onoff = %d\n", onoff);
    if (onoff)
    {
        pinctrl_select_state(pinctrl1, power_outputh);
    }
    else
    {
        pinctrl_select_state(pinctrl1, power_outputl);
    }
#if  (defined(CONFIG_PROJECT_V3991) || defined(CONFIG_PROJECT_V3995))
}
#endif
...
if (get_incell_lcm_connect()==false){
#endif
    power_outputl = pinctrl_lookup_state(pinctrl1, "state_power_outputl");
    if (IS_ERR(power_outputl))
    {
        ret = PTR_ERR(power_outputl);
        dev_err(&pdev->dev, "fwq Cannot find touch pinctrl state_power_outputl!\n");
        return ret;
    }
    power_outputh = pinctrl_lookup_state(pinctrl1, "state_power_outputh");
    if (IS_ERR(power_outputl))
    {
        ret = PTR_ERR(power_outputl);
        dev_err(&pdev->dev, "fwq Cannot find touch pinctrl state_power_outputh!\n");
        return ret;
    }
#if (defined(CONFIG_PROJECT_V3991) || defined(CONFIG_PROJECT_V3995))
}
...
#ifdef CONFIG_USE_FB_EARLY_EVENT_BLANK
	if (FB_EVENT_BLANK == event)
	{
		TPD_DMESG("HJDDbgTpLcd, tpd_fb_notifier_callback, FB_EVENT_BLANK \n");		
	}
	else if (FB_EARLY_EVENT_BLANK == event)
	{
		TPD_DMESG("HJDDbgTpLcd, tpd_fb_notifier_callback, FB_EARLY_EVENT_BLANK \n");
	}
#endif
