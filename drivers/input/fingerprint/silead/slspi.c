...
{	
	...
	{		
		...
		//mdelay(20);
		//printk("---20151203--read_sls_id--000000000000--silead finger chip id = %02x%02x%02x%02x\n",
		//	fc_read[3],fc_read[2],fc_read[1],fc_read[0]);

		val = spidev_read_reg(fp_spidev,0xfc);
	
		printk("---20151203--read_sls_id--11111--silead spidev_read_reg val = %08x\n",val);

	//printk("---20151203--read_sls_id--22222222--silead finger chip id = %02x%02x%02x%02x\n",
...
static unsigned char silead_rx[8] = {0};
static  ssize_t sl_fp_read(struct spidev_data *spidev, u8 addr, u8 *pdata, size_t len)
{
    ssize_t	status;
    uint32_t Rd_Data;
...
static void fp_ldo_enable(int e,struct spidev_data *pdev)
{
	if(pdev->fp_enable_low == NULL 
           || pdev->fp_enable_high == NULL) {
             printk("[spi_device] fp_ldo_enable: ~~~~~return~~~~~~~\n");
             return;
        }

	mutex_lock(&efsa120s_set_gpio_mutex);
        printk("[spi_device] fp_ldo_enable:%d\n", e);
...
static int spidev_probe(struct spi_device *spi)
{
   ...
   // spi_setup(spidev->spi);
	...
	//kfree(spidev);
//printk("20151203 gf_read_chip_id probe end  \n");
	          regulator_disable(spidev->reg);
		  return ret;
	  } 
