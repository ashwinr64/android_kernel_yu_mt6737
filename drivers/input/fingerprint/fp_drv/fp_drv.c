...
int full_fp_chip_info(const char *info)
{
	...
	for(i = 0; *(info +i) != 0; i++)
	{
		if(info[i] >'0' && info[i] <'9' && isVersionFormat == 0)
		{
			isVersionFormat = 1;
			j=i;
		}
 
		if(isVersionFormat == 1)
		{
			m_dev_info[i-j] = *(info +i);
		}
	}
...
{
        printk("---yokey---fingerprint info_show");
	if(has_exist) 
	{
		printk("---yokey---fingerprint info_show when has_exist");
