...
static void lcm_get_params(LCM_PARAMS *params)
{
		...
		params->dsi.horizontal_sync_active				= 10; //20; //8;
		params->dsi.horizontal_backporch				= 10; //20; //50;
		params->dsi.horizontal_frontporch				= 10; //20; //46;
...
static unsigned int lcm_compare_id(void)
{
	...
	MDELAY(2);
        ...
        LCM_DBG("JD9161_COE_TN:  id_high =%x\n", id_high);
        MDELAY(2);
        read_reg_v2(0xDB, buffer,1);
        id_midd = buffer[0]; ///////////////////////0x06
        LCM_DBG("JD9161_COE_TN:  id_midd =%x\n", id_midd);
        MDELAY(2);
        read_reg_v2(0xDC, buffer,1);
        id_low = buffer[0]; ////////////////////////0x04
        LCM_DBG("JD9161_COE_TN:  id_low =%x\n", id_low);

        //	id = id_high;
        id = (id_high << 8) | id_midd;
        LCM_DBG("JD9161_COE_TN: id = %x\n", id);
        
#ifndef AUXADC_LCM_VOLTAGE_CHANNEL
        LCM_DBG("JD9161_COE_TN: no need to check id pin voltage, id = %x(target=%x)\n", id, LCM_ID);
        return (LCM_ID == id) ? 1 : 0;
#else
    if (LCM_ID != id)
    {
        LCM_DBG("JD9161_COE_TN: not match, id = %x\n", id);        
        return 0;
    }
...
LCM_DRIVER jd9161_fwvga_dsi_vdo_coe_tn_lcm_drv = 
{
    .name			= "jd9161_fwvga_dsi_vdo_coe_tn",
