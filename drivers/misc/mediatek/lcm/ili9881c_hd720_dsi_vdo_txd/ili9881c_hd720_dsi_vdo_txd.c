...
#define TXD_LCM_MIN_VOL 0
#define TXD_LCM_MAX_VOL 500
...
static void lcm_get_params(LCM_PARAMS *params)
{
    ...
    params->dsi.vertical_backporch                   =24; //6;
    params->dsi.vertical_frontporch                   =24;//6;
		params->dsi.vertical_active_line = FRAME_HEIGHT; 

    params->dsi.horizontal_sync_active                = 10;//16;///////////////20 20 4  20  14  6
    params->dsi.horizontal_backporch                = 76;//15;
    params->dsi.horizontal_frontporch                = 76;//6;
		params->dsi.horizontal_active_pixel = FRAME_WIDTH;

		// Bit rate calculation
		//params->dsi.PLL_CLOCK=205;
		params->dsi.PLL_CLOCK=285;//215;  //LINE </EGAFM-297> <change the mipi clock to reduce disturbing the wifi> <20160413> panzaoyan
...
static unsigned int lcm_compare_id(void)
{
	...
	rc = IMM_GetOneChannelValue(AUXADC_LCD_ID_CHANNEL, data, &tmp);
		...
		if((TXD_LCM_MIN_VOL <= iVoltage) && (iVoltage < TXD_LCM_MAX_VOL))
				return 1;
			else
				return 0;
		}
...
LCM_DRIVER ili9881c_hd720_dsi_vdo_txd_lcm_drv = 
{
