...
#define LCM_ID       (0xB29806) // Jiangde ILI9806_TXD_IPS
...
static unsigned int lcm_compare_id(void)
{
    ...
    if(res < 0)
    {
        LCM_DBG("ILI9806_TXD: [adc_uboot]: get data error\n");
    }
    
    lcm_vol = data[0]*1000+data[1]*10;
    LCM_DBG("ILI9806_TXD: [adc_uboot]: lcm_vol= %d\n",lcm_vol);
#endif
    ...
    id_super = buffer[0]; ///////////////////////0x98
    LCM_DBG("ILI9806_TXD:  id_super =%x\n", id_super);
    ...
    LCM_DBG("ILI9806_TXD:  id_high =%x\n", id_high);
    MDELAY(2);
    read_reg_v2(0x01, buffer,1);
    id_midd = buffer[0]; ///////////////////////0x06
    LCM_DBG("ILI9806_TXD:  id_midd =%x\n", id_midd);
    MDELAY(2);
    // read_reg_v2(0x02, buffer,1);
    read_reg_v2(0xDA, buffer,1);
    id_low = buffer[0]; ////////////////////////0x04
    LCM_DBG("ILI9806_TXD:  id_low =%x\n", id_low);

    //	id = id_high;
    id = (id_super << 16) | (id_high << 8) | id_midd;
    LCM_DBG("ILI9806_TXD: id = %x\n", id);

#ifndef AUXADC_LCM_VOLTAGE_CHANNEL
    LCM_DBG("ILI9806_TXD, No need to check id pin voltage, id = %x(target=%x)\n", id, LCM_ID);
    return (LCM_ID == id) ? 1 : 0;
#else
    if (LCM_ID != id)
    {
        LCM_DBG("ILI9806_TXD, not match, id = %x\n", id);
        return 0;
    }
    ...
    {
        LCM_DBG("ILI9806_TXD: [adc_uboot]: find lcm id = %x lcm_vol = %d\n",id,lcm_vol);
        return 1;
    }else
    {
        return 0;
