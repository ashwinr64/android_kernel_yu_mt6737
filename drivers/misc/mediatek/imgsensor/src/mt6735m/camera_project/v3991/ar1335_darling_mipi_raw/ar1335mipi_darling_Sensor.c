...
#define PFX "ar1335_camera_darling_sensor"
...
extern bool is_darling_camera(void);
static DEFINE_SPINLOCK(imgsensor_drv_lock);
//#define AR1335_OTP
#define  AR1335_OTP_TEST_KEY
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = AR1335_DARLING_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static void capture_setting(kal_uint16 currefps)
{
  ...
  /// stop_streaming
	...
	////// vt_clk = 438mhz pclk = mhz data rate = 876mhz  4208*3120@24fps
	write_cmos_sensor_2byte(0x31AE, 0x0204); 	// SERIAL_FORMAT
	write_cmos_sensor_2byte(0x0300, 0x0004); 	// VT_PIX_CLK_DIV
	write_cmos_sensor_2byte(0x0302, 0x0001); 	// VT_SYS_CLK_DIV
	write_cmos_sensor_2byte(0x0304, 0x0404); 	// PRE_PLL_CLK_DIV
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            *sensor_id = return_sensor_id();
		    ...
		    if(is_darling_camera())
                {   
                    Darling_AR1335_read_OTP();
                    return ERROR_NONE;
                }
		    else 
		    {
...
static kal_uint32 open(void)
{
    ...
    sensor_init();
    #if 0
    if ((imgsensor_info.sensor_id != sensor_id)||(!(is_darling_camera())))
	    return ERROR_SENSOR_CONNECT_FAIL;
    #endif
...
UINT32 AR1335_DARLING_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
