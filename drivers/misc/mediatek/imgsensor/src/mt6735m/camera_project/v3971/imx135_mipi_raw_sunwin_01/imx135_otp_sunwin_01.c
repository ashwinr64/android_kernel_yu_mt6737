...
//#define IMX135_SUNWIN_01_OTP(fmt,arg...)  pr_err(PFX "[%s] " format, __FUNCTION__, ##args)
...
**************************************************************************************************/
 BYTE sunwin_01_get_otp_module_id(void)
{
    BYTE module_id = 0;
	BYTE id_zone=0;
...
**************************************************************************************************/
BYTE sunwin_01_get_otp_lens_id(void)
{
	BYTE lens_id = 0;
	BYTE id_zone=0;
	if(get_otp_flag(0x02)) //page 1 and page 2 for awb
...
#ifdef imx135_otp_sunwin_01_debug
		val = i*64+j;
...
void otp_clear_flag_sunwin_01(void){
...
#endif
USHORT sunwin_01_af_inf;
USHORT sunwin_01_af_mac;
//liaoshuang add for imx135 otp
bool otp_update_sunwin_01(BYTE update_sensor_otp_awb, BYTE update_sensor_otp_lsc)
{
...
//af data 
	if(start_read_otp(0x0f))
    {
		...
		sunwin_01_af_mac = (read_cmos_sensor(0x3b17) << 8) | (read_cmos_sensor(0x3b18));
	else 
		sunwin_01_af_mac = (read_cmos_sensor(0x3b07) << 8) | (read_cmos_sensor(0x3b08));
	if(read_cmos_sensor(0x3B34))
		sunwin_01_af_inf = (read_cmos_sensor(0x3b37) << 8) | (read_cmos_sensor(0x3b38));	
	else		
		sunwin_01_af_inf = (read_cmos_sensor(0x3b27) << 8) | (read_cmos_sensor(0x3b28));
	...
	LOG_INF("sunwin_01_af_mac = %d,sunwin_01_af_inf = %d\n",sunwin_01_af_mac,sunwin_01_af_inf);
}
...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "IMX135_SUNWIN_01_CAM_CAL_DRV"
...
u8 imx135_sunwin_01_id_data[]= {0x10,0xff,0x00,0x40,0x89};
static int selective_read_region(u32 offset, BYTE* data,u16 i2c_id,u32 size)
{    
    printk("[IMX135_SUNWIN_01_CAM_CAL] selective_read_region offset =%d size %d data read = %d\n", offset,size, *data);
if(size ==2 ){
	if(offset == 7){
		memcpy((void *)data,(void *)&sunwin_01_af_inf,size);
	}
	
	else if(offset == 9){
		 
		memcpy((void *)data,(void *)&sunwin_01_af_mac,size);

	}
}
if(size == 4){
		memcpy((void *)data,(void *)&imx135_sunwin_01_id_data[1],size);
...
#endif
{
    ...
    else
    {
        ...
        if(NULL == pBuff)
        {
            LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] ioctl allocate mem failed\n");
            return -ENOMEM;
        }
        ...
        if(_IOC_WRITE & _IOC_DIR(a_u4Command))
        {
            ...
            {    //get input structure address
                kfree(pBuff);
                LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] ioctl copy from user failed\n");
                return -EFAULT;
            }
        }
    }
...
//fix warning MSG     LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] init Working buffer address 0x%x  command is 0x%08x\n", pWorkingBuff, a_u4Command);
    ...
    LOG_INF("[LOG_INF] CAM_CALIOC_S_WRITE \n");        
        ...
        case CAM_CALIOC_G_READ:
            LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] Read CMD \n");
#ifdef CAM_CALGETDLT_DEBUG            
            do_gettimeofday(&ktv1);
#endif 
            LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] offset %d \n", ptempbuf->u4Offset);
            LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] length %d \n", ptempbuf->u4Length);
          // LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] Before read Working buffer address 0x%x \n", pWorkingBuff);
...
if(ptempbuf->u4Length == 2){
	   ...
	   i4RetValue = selective_read_region(ptempbuf->u4Offset, pWorkingBuff, 0x20, ptempbuf->u4Length);
          ...
          // LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] After read Working buffer address 0x%x \n", pWorkingBuff);
            ...
            break;
        default :
      	     LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] No CMD \n");
            i4RetValue = -EPERM;
        break;
    }
    ...
    if(_IOC_READ & _IOC_DIR(a_u4Command))
    {
        ...
        LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] to user length %d \n", ptempbuf->u4Length);
		...
		if(copy_to_user((u8 __user *) ptempbuf->pu1Params , (u8 *)pWorkingBuff , ptempbuf->u4Length))
        {
            kfree(pBuff);
            kfree(pWorkingBuff);
            LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] ioctl copy to user failed\n");
            return -EFAULT;
        }
    }
...
//#else
    ...
    //{
    //    LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] Fail to enable analog gain\n");
    //    return -EIO;
    //}
//#endif	
...
inline static int RegisterCAM_CALCharDrv(void)
{
    ...
    {
        LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] Allocate device no failed\n");
    ...
    {
        LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] Register device no failed\n");
    ...
    if(NULL == g_pCAM_CAL_CharDrv)
    {
        ...
        LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] Allocate mem for kobject failed\n");
    ...
    {
        LOG_INF("[IMX135_SUNWIN_01_CAM_CAL] Attatch file operation failed\n");
    ...
    CAM_CAL_class = class_create(THIS_MODULE, "IMX135_SUNWIN_01_CAM_CALdrv");
...
static int __init CAM_CAL_i2C_init(void)
{
   ...
   // i2c_register_board_info(CAM_CAL_I2C_BUSNUM, &kd_cam_cal_dev, 1);
	 ...
	 int i4RetValue = 0;
    LOG_INF("[IMX135_SUNWIN_01_CAM_CAL]\n");
   //Register char driver
	i4RetValue = RegisterCAM_CALCharDrv();
    if(i4RetValue){
 	   LOG_INF(" [IMX135_SUNWIN_01_CAM_CAL] register char device failed!\n");
	   return i4RetValue;
	}
	LOG_INF(" [IMX135_SUNWIN_01_CAM_CAL] Attached!! \n");
...
LOG_INF(" IMX135_SUNWIN_01_CAM_CAL  Attached Pass !! \n");
    return 0;
}
