...
//	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_B,//SENSOR_OUTPUT_FORMAT_RAW_Gr,//SENSOR_OUTPUT_FORMAT_RAW_R,,//SENSOR_OUTPUT_FORMAT_RAW_Gr,
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gb,//SENSOR_OUTPUT_FORMAT_RAW_Gr,//SENSOR_OUTPUT_FORMAT_RAW_R,,//SENSOR_OUTPUT_FORMAT_RAW_Gr,
	.mclk = 24,
...
#ifdef S5K5E8_DARLING_OTP_USE
if(OTP_KEY == 2 ||OTP_KEY == 3)
{
    if ((OTP_KEY>>1) & 1)
    {
       S5K5E8_Darling_update_otp_wb();
    }
}
#else
    S5K5E8_Darling_update_otp_wb();
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x3400,0x00);//write_cmos_sensor(0x3400,0x01); //shade_bypass LSC off
	//Streaming on
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 150){
		...
		write_cmos_sensor(0x0203,0XDE);
		write_cmos_sensor(0x3303,0X02);
		write_cmos_sensor(0x3400,0x00);//write_cmos_sensor(0x3400,0X01);
		write_cmos_sensor(0x3C0D,0X04);
		write_cmos_sensor(0x0100,0X01);
		write_cmos_sensor(0x3C22,0X00);
		write_cmos_sensor(0x3C22,0X00);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {	
                    Read_S5K5E8_Darling_OTP_Data(); //liaoshuang add otp read
