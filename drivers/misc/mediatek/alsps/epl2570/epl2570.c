...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    dynk_low_offset =epl_sensor.ps.low_threshold;
    dynk_high_offset =epl_sensor.ps.high_threshold;
#endif /*PS_DYN_K*/
...
long raw_convert_to_lux(u16 raw_data)
{
    ...
    //lux = c_gain * epl_sensor.als.dyn_intt_raw;
     lux = epl_sensor.als.dyn_intt_raw;
	...
	APS_LOG("step_value:anti_count =  %d orient_flag=%d \n",anti_count,orient_flag);
		...
		{	
		   ...
		   orient_flag=0;
                ...
                APS_LOG("niu anti ALS: idx = %d, %05d => %05d\n",idx, als, obj->hw->als_value[idx]);
...
static int epl_sensor_get_als_value(struct epl_sensor_priv *obj, u16 als)
{
    ...
    switch(epl_sensor.als.report_type)
    {
		...
		case CMC_BIT_DYN_INT:
            ...
            {
            #ifdef ALS_TINNO_DYN
	           	als_value_tinno= tinno_get_als_value(obj,als);
			APS_LOG("niu anti als_value_tinno = %d \n",als_value_tinno);
...
void epl_sensor_dynk_thd_polling_work(struct work_struct *work)
{
...
#endif
    {
		...
		if(g_ps_base_value>epl_sensor.ps.data.data)
            ...
            if(enable_ps == true)
            { 
            APS_LOG("[%s]:dynk_high_offset = %d, dynk_low_offset = %d\n", __func__, dynk_high_offset, dynk_low_offset);
