...
//BEGIN<DHBAN-2><20161014><read efuse status>wangyanhui
// path:  /sys/bus/platform/drivers/mt-gpio/efuse_state
extern int sec_schip_enabled(void);
static ssize_t efuse_status_info_show(struct device_driver *ddri, char *buf)
{
	int efuse_temp ;
	efuse_temp = sec_schip_enabled();
	return sprintf(buf, "%d\n", efuse_temp);
}
static DRIVER_ATTR(efuse_state, 0664, efuse_status_info_show,  NULL);

static void create_efuse_attr(void);
//END<DHBAN-2><20161014><read efuse status>wangyanhui
...
static int mt_gpio_probe(struct platform_device *dev)
{
	...
	create_efuse_attr();//LINE<DHBAN-2><20161014><read efuse status>wangyanhui
...
{
	int err;
	err = driver_create_file(&gpio_driver.driver, &driver_attr_efuse_state);
	if (err) {
		driver_remove_file(&gpio_driver.driver,&driver_attr_efuse_state);
	}
}
//END<DHBAN-2><20161014><read efuse status>wangyanhui
