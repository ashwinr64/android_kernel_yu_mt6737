...
#if (defined(CONFIG_PROJECT_V3991) || defined(CONFIG_PROJECT_V3995))
...
#if (defined(CONFIG_PROJECT_V3991) || defined(CONFIG_PROJECT_V3995))
struct pinctrl_state  *lcm_power_outputl = NULL, *lcm_power_outputh = NULL; //fengyongfei add 0927
struct pinctrl_state  *lcm_cs_outputl = NULL, *lcm_cs_outputh = NULL; 
#endif
...
int get_lcm_id_status(void)
{
	...
	printk("get lcm id gpio:%d, ret:%d\n", lcm_id_gpio, ret);
	return ret;
}
#if (defined(CONFIG_PROJECT_V3991) || defined(CONFIG_PROJECT_V3995))
void lcm_power_ldo(int onoff)
{
	if(disptepinctrl != NULL){
	    //printk("lcm_power_ldo onoff = %d\n", onoff);
	    if (onoff)
	    {
	        pinctrl_select_state(disptepinctrl, lcm_power_outputh);
	    }
	    else
	    {
	        pinctrl_select_state(disptepinctrl, lcm_power_outputl);
	    }
	}else{
...
void lcm_cs_pin(int onoff)
{
   if(disptepinctrl != NULL){

	    //printk("lcm_cs_pin onoff = %d\n", onoff);
	    if (onoff)
	    {
	        pinctrl_select_state(disptepinctrl, lcm_cs_outputh);
	    }
	    else
	    {
	        pinctrl_select_state(disptepinctrl, lcm_cs_outputl);
	    }
     }else{
		printk("lcm cs disptepinctrl fail \n");
	  }
}
#endif
...
static int DispTE_use_gpio_probe(struct platform_device *pdev)
{
	...
	if (ret)
		printk(" get lcm_id_gpio fail, ret = %d\n", ret);  

	#if (defined(CONFIG_PROJECT_V3991) || defined(CONFIG_PROJECT_V3995))
	   lcm_power_outputl = pinctrl_lookup_state(disptepinctrl, "lcm_power_outputl");
	   if (IS_ERR(lcm_power_outputl))
		{
			ret = PTR_ERR(lcm_power_outputl);
			dev_err(&pdev->dev, "fwq Cannot find lcm pinctrl state_power_outputl!\n");
			return ret;
		}
		lcm_power_outputh = pinctrl_lookup_state(disptepinctrl, "lcm_power_outputh");
		if (IS_ERR(lcm_power_outputl))
		{
			ret = PTR_ERR(lcm_power_outputl);
			dev_err(&pdev->dev, "fwq Cannot find lcm pinctrl state_power_outputh!\n");
			return ret;
		}

		lcm_cs_outputl = pinctrl_lookup_state(disptepinctrl, "lcm_cs_outputl");
	   if (IS_ERR(lcm_cs_outputl))
		{
			ret = PTR_ERR(lcm_cs_outputl);
			dev_err(&pdev->dev, "fwq Cannot find lcm pinctrl lcm_cs_outputl!\n");
			return ret;
		}
		lcm_cs_outputh = pinctrl_lookup_state(disptepinctrl, "lcm_cs_outputh");
		if (IS_ERR(lcm_cs_outputh))
		{
			ret = PTR_ERR(lcm_cs_outputh);
			dev_err(&pdev->dev, "fwq Cannot find lcm pinctrl lcm_cs_outputh!\n");
			return ret;
		}
