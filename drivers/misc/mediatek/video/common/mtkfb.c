...
static bool is_incell_lcm = false;
static void  set_incell_lcm_connect(void)
{
	DISPERR("mtkfb lcm = %s\n",mtkfb_find_lcm_driver());

	if (!strcmp(mtkfb_find_lcm_driver(), "td4100_hd720_dsi_vdo"))
	{
		is_incell_lcm = true;
	}else{
		is_incell_lcm = false;
...
bool get_incell_lcm_connect(void)
{
	DISPERR("mtkfb is_incell_lcm = %d\n",is_incell_lcm);
	return is_incell_lcm;
}
#endif
