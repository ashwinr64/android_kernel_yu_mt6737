...
#define CAM_CALINF(format, args...)    pr_debug(PFX "[%s] " format, __FUNCTION__, ##args)
...
const u8 s5k3m2_otp_version[] = {0x8C,0x6b,0xff,0x83,0x54};
...
#endif /*CONFIG_COMPAT*/
extern int  S5K3M2_vcm_data[2];
static void s5k3m2_read_otp(u32 offset,BYTE* data,u32 size)
{
	if((0xa2f==offset) &&(1==size))
	{
		*data = 0xE;//AWBAFConfig =0xE mean,do af  calibration
	}
        else if((0xa30==offset) &&(2==size))
            {
            memcpy((void *)data,(void *)&S5K3M2_vcm_data[0],size);
        }
        else if ((0xa32==offset) &&(2==size))
            {
           memcpy((void *)data,(void *)&S5K3M2_vcm_data[1],size);     
        }
}

static int selective_read_region(u32 offset, BYTE* data,u32 size)
{
    CAM_CALINF("goto_selective_read_region");
	if((0==offset) &&(4==size))
	{
		memcpy((void *)data,(void *)&s5k3m2_otp_version[offset],size);
            CAM_CALINF("selective_read_region.S5K3M2_vcm_data[0]= %d,S5K3M2_vcm_data[1]= %d",S5K3M2_vcm_data[0],S5K3M2_vcm_data[1]);
	}
	else
	{   
            s5k3m2_read_otp(offset,data,size);
	}
   return   size;
}
